...
fn generate_random_record() -> Record {
    ...
    let user = User {
        ...
        age: 30,
        partner: netabase_store::relational::RelationalLink::new_dehydrated(UserID(
            "none".to_string(),
        )),
...
fn bench_record_store(c: &mut Criterion) {
    ...
    for size in sizes.iter() {
            ...
            b.iter_batched(
                || {
                    let records: Vec<Record> =
                        (0..size).map(|_| generate_random_record()).collect();
        ...
        group.bench_with_input(
            BenchmarkId::new("Put_ContentAddressed", size),
            size,
            |b, &size| {
                b.iter_batched(
                    || {
                        let records: Vec<Record> = (0..size)
                            .map(|_| generate_random_content_addressed_record())
                            .collect();
                        let name =
                            format!("bench_ca_record_put_{}_{}", size, rand::random::<u64>());
                        let store =
        ...
        group.bench_with_input(
            BenchmarkId::new("Get_ContentAddressed", size),
            size,
            |b, &size| {
                b.iter_batched(
                    || {
                        let records: Vec<Record> = (0..size)
                            ...
                            .collect();
                        let name =
                            format!("bench_ca_record_get_{}_{}", size, rand::random::<u64>());
                        let store =
                            create_test_db::<Definition>(&name).expect("Failed to create DB");
                        let mut record_store = Libp2pRedbStore::new(store, local_id);
        ...
        group.bench_with_input(
            BenchmarkId::new("Put_ContentAddressed_Fast", size),
            size,
            |b, &size| {
                b.iter_batched(
                    || {
                        let records: Vec<Record> = (0..size)
                            .map(|_| generate_random_content_addressed_fast_record())
                            .collect();
                        let name = format!(
                            "bench_ca_fast_record_put_{}_{}",
                            size,
                            rand::random::<u64>()
                        );
                        let store =
        ...
        group.bench_with_input(
            BenchmarkId::new("Get_ContentAddressed_Fast", size),
            size,
            |b, &size| {
                b.iter_batched(
                    || {
        ...
        group.bench_with_input(
            BenchmarkId::new("Put_ContentAddressed_Crypto", size),
            size,
            |b, &size| {
                b.iter_batched(
                    || {
                        let records: Vec<Record> = (0..size)
                            .map(|_| generate_random_content_addressed_crypto_record())
                            .collect();
                        let name = format!(
                            "bench_ca_crypto_record_put_{}_{}",
                            size,
                            rand::random::<u64>()
                        );
                        let store =
        ...
        group.bench_with_input(
            BenchmarkId::new("Get_ContentAddressed_Crypto", size),
            size,
            |b, &size| {
                b.iter_batched(
                    || {
