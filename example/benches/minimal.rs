...
pub mod minimal_def {
    ...
    #[derive(
        NetabaseModel, Debug, Clone, Serialize, Deserialize, PartialEq, Eq, Hash, PartialOrd, Ord,
    )]
    pub struct Item {
        #[primary_key]
        pub id: String,
                ...
                b.iter_batched(
                    ...
                    |(store, items)| {
                        let txn = store.begin_write().expect("Failed to begin write");
                        {
                            let mut tables = txn
                                .prepare_model::<Item>()
                                .expect("Failed to prepare tables");
                            for item in &items {
            ...
            |b, _| {
                b.iter_batched(
                    || {
                        ...
                        {
                            let mut tables =
                                txn.prepare_model::<Item>().expect("Failed to prepare");
                            for item in &items {
                                item.create_entry(&mut tables).expect("Failed to insert");
                            }
                        }
