//! Memory subscription tree test
//!
//! This test verifies that subscription trees work correctly with the memory store.
...
#[test]
fn test_memory_subscription_basic_operations() {
    let store = MemoryStore::<TestDef>::new();
...
#[test]
fn test_memory_subscription_multiple_types() {
    let store = MemoryStore::<TestDef>::new();
...
#[test]
fn test_memory_subscription_clear() {
    let store = MemoryStore::<TestDef>::new();
    let mut sub_tree = store.open_subscription_tree(TestSubscriptions::UserNotifications);
