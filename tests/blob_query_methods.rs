...
fn test_blob_storage_single_large_field() -> NetabaseResult<()> {
    ...
    // Read back and verify blob data integrity
    {
        ...
        println!("Read user with {} bytes of blob data", retrieved_user.bio.data.len());
        assert_eq!(retrieved_user.bio.data.len(), 100_000, "Blob data should be intact");
        assert_eq!(retrieved_user.bio.data, large_data, "Blob data should match original");
...
fn test_blob_storage_multiple_users() -> NetabaseResult<()> {
    ...
    // Verify all users can be read back with intact blob data
    {
        ...
        for i in 0..5 {
            ...
            assert_eq!(user.another.0.len(), 30_000, "User {} another should be 30KB", i);
...
fn test_blob_storage_very_large_data() -> NetabaseResult<()> {
    ...
    // Verify large blobs are correctly stored and retrieved
    {
        ...
        println!("Retrieved user with {}KB + {}KB of blob data", 
        ...
        assert_eq!(retrieved_user.another.0, large_another, "Another blob should match");
