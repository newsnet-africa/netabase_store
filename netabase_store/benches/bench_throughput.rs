...
impl Value for UserSecondaryKeys {
    ...
    where
        Self: 'a,
    {
        Self::try_from(data.to_vec()).unwrap()
    }
    fn as_bytes<'a, 'b: 'a>(value: &'a Self::SelfType<'b>) -> Self::AsBytes<'a> {
        Cow::Owned(value.clone().try_into().unwrap())
    }
    fn type_name() -> TypeName {
        TypeName::new("UserSecondaryKeys")
    }
}
impl Key for UserSecondaryKeys {
    fn compare(data1: &[u8], data2: &[u8]) -> std::cmp::Ordering {
        let v1: UserSecondaryKeys = bincode::decode_from_slice(data1, bincode::config::standard())
            .unwrap()
            .0;
        let v2: UserSecondaryKeys = bincode::decode_from_slice(data2, bincode::config::standard())
            .unwrap()
            .0;
...
impl TryFrom<UserSecondaryKeys> for Vec<u8> {
    type Error = bincode::error::EncodeError;
    fn try_from(v: UserSecondaryKeys) -> Result<Self, Self::Error> {
...
impl Value for UserRelationalKeys {
    ...
    fn type_name() -> TypeName {
        TypeName::new("UserRelationalKeys")
    }
}
impl Key for UserRelationalKeys {
    fn compare(d1: &[u8], d2: &[u8]) -> std::cmp::Ordering {
        let v1: UserRelationalKeys = bincode::decode_from_slice(d1, bincode::config::standard())
            .unwrap()
            .0;
        let v2: UserRelationalKeys = bincode::decode_from_slice(d2, bincode::config::standard())
            .unwrap()
            .0;
        match (v1, v2) {
            (UserRelationalKeys::BestFriend(id1), UserRelationalKeys::BestFriend(id2)) => {
                id1.cmp(&id2)
            }
        }
    }
...
impl TryFrom<Vec<u8>> for UserRelationalKeys {
    ...
    fn try_from(v: Vec<u8>) -> Result<Self, Self::Error> {
        Ok(bincode::decode_from_slice(&v, bincode::config::standard())?.0)
    }
}
impl TryFrom<UserRelationalKeys> for Vec<u8> {
    type Error = bincode::error::EncodeError;
    fn try_from(v: UserRelationalKeys) -> Result<Self, Self::Error> {
        bincode::encode_to_vec(v, bincode::config::standard())
    }
}
...
impl Value for UserSubscriptions {
    ...
    fn type_name() -> TypeName {
        TypeName::new("UserSubscriptions")
    }
}
...
impl TryFrom<UserSubscriptions> for Vec<u8> {
    type Error = bincode::error::EncodeError;
    fn try_from(v: UserSubscriptions) -> Result<Self, Self::Error> {
        bincode::encode_to_vec(v, bincode::config::standard())
    }
}
...
impl NetabaseModelKeyTrait<Definitions, User> for UserKeys {
    ...
    fn secondary_keys(model: &User) -> Vec<Self::SecondaryEnum> {
        vec![UserSecondaryKeys::Email(UserEmail(model.email.clone()))]
    }
...
impl NetabaseModelTrait<Definitions> for User {
    ...
    fn get_secondary_keys(&self) -> Self::SecondaryKeys {
        UserSecondaryKeysIter {
            iter: vec![UserSecondaryKeys::Email(UserEmail(self.email.clone()))].into_iter(),
        }
    }
...
impl RedbNetabaseModelTrait<Definitions> for User {
    fn definition<'a>(_db: &RedbStore<Definitions>) -> TableDefinition<'a, Self::PrimaryKey, Self> {
        TableDefinition::new(Self::MODEL_TREE_NAME.as_ref())
    }
    fn secondary_key_table_name(k: UserSecondaryKeysDiscriminants) -> String {
        format!("User_sec_{}", k.as_ref())
    }
    fn relational_key_table_name(k: UserRelationalKeysDiscriminants) -> String {
        format!("User_rel_{}", k.as_ref())
    }
    fn subscription_key_table_name(k: UserSubscriptionsDiscriminants) -> String {
        format!("User_sub_{}", k.as_ref())
    }
...
impl RedbModelAssociatedTypesExt<Definitions> for DefinitionModelAssociatedTypes {
    fn insert_model_into_redb(
        ...
        match (self, key) {
            (
                DefinitionModelAssociatedTypes::UserModel(m),
        ...
        match (self, primary_key_ref) {
            (
                ...
                let mut table = txn.open_table(
                    TableDefinition::<UserSecondaryKeys, UserId>::new(table_name),
                )?;
                table.insert(sk, pk)?;
                Ok(())
            }
    ...
    fn insert_relational_key_into_redb(
        ...
        match (self, primary_key_ref) {
            (
                ...
                let mut table = txn.open_table(
                    TableDefinition::<UserRelationalKeys, UserId>::new(table_name),
                )?;
                table.insert(rk, pk)?;
                Ok(())
            }
    ...
    fn insert_hash_into_redb(
        ...
        match primary_key_ref {
            DefinitionModelAssociatedTypes::UserPrimaryKey(pk) => {
                let mut table =
                    txn.open_table(TableDefinition::<[u8; 32], UserId>::new(table_name))?;
    ...
    fn insert_subscription_into_redb(
        ...
        match primary_key_ref {
            DefinitionModelAssociatedTypes::UserPrimaryKey(pk) => {
                let mut table =
                    txn.open_table(TableDefinition::<UserId, [u8; 32]>::new(table_name))?;
    ...
    fn delete_model_from_redb(&self, _: &WriteTransaction, _: &str) -> NetabaseResult<()> {
        Ok(())
    }
    fn delete_subscription_from_redb(&self, _: &WriteTransaction, _: &str) -> NetabaseResult<()> {
        Ok(())
    }
}
...
impl SledModelAssociatedTypesExt<Definitions> for DefinitionModelAssociatedTypes {
    fn insert_model_into_sled(
        ...
        match (self, key) {
            (
                ...
                tree.insert(
                    bincode::encode_to_vec(pk, config)?,
                    bincode::encode_to_vec(m, config)?,
                )?;
                Ok(())
            }
            _ => Err(NetabaseError::Other("Mismatch".into())),
    ...
    fn insert_secondary_key_into_sled(
        ...
        match (self, primary_key_ref) {
            (
                ...
                let tree = db.open_tree(tree_name)?;
                tree.insert(
                    bincode::encode_to_vec(sk, config)?,
                    bincode::encode_to_vec(pk, config)?,
                )?;
                Ok(())
            }
    ...
    fn insert_relational_key_into_sled(
        ...
        match (self, primary_key_ref) {
            (
                ...
                let tree = db.open_tree(tree_name)?;
                tree.insert(
                    bincode::encode_to_vec(rk, config)?,
                    bincode::encode_to_vec(pk, config)?,
                )?;
                Ok(())
            }
    ...
    fn insert_hash_into_sled(
        ...
        match primary_key_ref {
            DefinitionModelAssociatedTypes::UserPrimaryKey(pk) => {
                ...
                tree.insert(
                    bincode::encode_to_vec(pk, config)?,
                    bincode::encode_to_vec(hash, config)?,
                )?;
                Ok(())
            }
            _ => Err(NetabaseError::Other("Mismatch".into())),
    ...
    fn delete_model_from_sled(&self, _: &sled::Db, _: &str) -> NetabaseResult<()> {
        Ok(())
    }
    fn delete_subscription_from_sled(&self, _: &sled::Db, _: &str) -> NetabaseResult<()> {
        Ok(())
    }
}
...
impl NetabaseDefinitionTrait for Definitions {
    ...
    type Permissions = NoPermissions;
}
...
impl TreeManager<Definitions> for Definitions {
    ...
    fn get_tree_name(d: &DefinitionsDiscriminants) -> Option<String> {
        match d {
            DefinitionsDiscriminants::User => Some("User".to_string()),
        }
    }
    fn get_secondary_tree_names(d: &DefinitionsDiscriminants) -> Vec<String> {
        match d {
            DefinitionsDiscriminants::User => vec!["User_Email".to_string()],
        }
    }
    fn get_relational_tree_names(d: &DefinitionsDiscriminants) -> Vec<String> {
        match d {
            DefinitionsDiscriminants::User => vec!["User_BestFriend".to_string()],
        }
    }
    fn get_subscription_tree_names(d: &DefinitionsDiscriminants) -> Vec<String> {
        match d {
            DefinitionsDiscriminants::User => vec!["User_Premium".to_string()],
        }
    }
}
...
fn generate_users(count: usize) -> Vec<User> {
    ...
    (0..count)
        .map(|i| User {
            ...
            best_friend: if i > 0 && rng.r#gen::<bool>() {
                Some((i - 1) as u64)
            } else {
                None
            },
...
fn bench_redb_ops(users: &[User]) -> NetabaseResult<()> {
    ...
    store.read(|txn| {
        let _pk = txn.get_pk_by_secondary_key::<User>(email_key.clone())?;
        Ok(())
    })?;
    println!(
        "  Redb Secondary Lookup (1 item): {:?}",
        start_sec.elapsed()
    );
    ...
    store.read(|txn| {
        let _ =
            txn.get_subscription_accumulator::<User>(UserSubscriptionsDiscriminants::Premium)?;
        Ok(())
    })?;
    println!("  Redb Subscription Accumulator: {:?}", start_sub.elapsed());
