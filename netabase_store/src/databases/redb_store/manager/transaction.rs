use crate::databases::redb_store::manager::RedbDefinitionManager;
...
{
    manager: &'a mut RedbDefinitionManager<R, D, P>,
    permission: P,
}

impl<'a, R, D, P> RedbMultiDefReadTxn<'a, R, D, P>
where
    R: DefinitionManagerTrait<Definition = D, Permissions = P, Backend = RedbStore<D>>,
    D: NetabaseDefinition,
    P: PermissionEnumTrait,
        ...
        IntoEnumIterator + std::hash::Hash + Eq + Debug + Clone,
{
    ...
    pub fn new(manager: &'a mut RedbDefinitionManager<R, D, P>, permission: P) -> Self {
        ...
        IntoEnumIterator + std::hash::Hash + Eq + Debug + Clone,
{
    ...
    where
        for<'b> F: FnOnce(&<RedbStore<D> as StoreTrait<D>>::ReadTxn<'b>) -> NetabaseResult<Ret>,
        D: 'static,
    {
...
/// Write transaction for multi-definition access with Redb backend
///
...
pub struct RedbMultiDefWriteTxn<'a, R, D, P>
where
    R: DefinitionManagerTrait<Definition = D, Permissions = P, Backend = RedbStore<D>>,
    D: NetabaseDefinition,
    P: PermissionEnumTrait,
...
impl<'a, R, D, P> RedbMultiDefWriteTxn<'a, R, D, P>
where
    R: DefinitionManagerTrait<Definition = D, Permissions = P, Backend = RedbStore<D>>,
    D: NetabaseDefinition,
    P: PermissionEnumTrait,
        ...
        IntoEnumIterator + std::hash::Hash + Eq + Debug + Clone,
{
    ...
    where
        F: FnOnce(&mut <RedbStore<D> as StoreTrait<D>>::WriteTxn) -> NetabaseResult<Ret>,
    {
    ...
    pub fn commit(self) -> NetabaseResult<()> {
        ...
        // by the store.write() calls above (redb auto-commits on drop)
