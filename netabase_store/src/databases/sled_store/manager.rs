...
/// Sled-specific definition manager
///
/// This wrapper provides Sled-specific functionality for the generic
...
/// # Example
/// ```ignore
/// let manager = SledDefinitionManager::<
///     RestaurantManager,
///     RestaurantDefinitions,
///     RestaurantPermissions
/// >::new("./data")?;
/// ```
pub struct SledDefinitionManager<R, D, P>
where
...
{
    pub(crate) inner: DefinitionManager<R, D, P, SledStore<D>>,
    _marker: PhantomData<R>,
}

impl<R, D, P> SledDefinitionManager<R, D, P>
where
    R: DefinitionManagerTrait<Definition = D, Permissions = P, Backend = SledStore<D>>,
    D: NetabaseDefinition,
    P: PermissionEnumTrait,
    ...
    <P as IntoDiscriminant>::Discriminant:
        IntoEnumIterator + std::hash::Hash + Eq + Debug + Clone,
{
    /// Create a new Sled definition manager
    ///
    /// # Arguments
    ...
    ///
    /// This opens the Sled database at the appropriate path and marks
    /// the definition as loaded.
    ///
    /// Note: Sled treats the path as a directory, so `store.db` becomes
    /// a directory containing the actual database files.
    ///
    /// # Arguments
    ...
    /// * `Err(SledError)` - If database opening failed
    ...
    pub fn load_definition(
        ...
        // Note: Sled treats this as a directory path
        let path = self
            .inner
            .root_path()
    ...
    where
        F: FnOnce(&transaction::SledMultiDefReadTxn<'_, R, D, P>) -> NetabaseResult<Ret>,
    {
        let txn = transaction::SledMultiDefReadTxn::new(self, permission);
        f(&txn)
    }
    ...
    where
        F: FnOnce(&mut transaction::SledMultiDefWriteTxn<'_, R, D, P>) -> NetabaseResult<Ret>,
    {
        let mut txn = transaction::SledMultiDefWriteTxn::new(self, permission);
    ...
    /// # Example
    /// ```ignore
    /// let mut manager = SledDefinitionManager::new("./data")?;
...
impl<R, D, P> Debug for SledDefinitionManager<R, D, P>
where
    R: DefinitionManagerTrait<Definition = D, Permissions = P, Backend = SledStore<D>>,
    D: NetabaseDefinition,
    P: PermissionEnumTrait,
        ...
        IntoEnumIterator + std::hash::Hash + Eq + Debug + Clone,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SledDefinitionManager")
