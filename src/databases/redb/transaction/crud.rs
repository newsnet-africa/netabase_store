...
//! txn.create_with_subscriptions(&user, Some(vec![topic1, topic2]))?;
//! ```
//!
//! ## Content-Addressed Models
//!
//! ```rust
//! // For immutable models where hash is pre-calculated
//! let hash = calculate_hash(&model);
//! txn.create_with_hash(&model, &hash)?;
//! ```
...
///
/// ```rust
/// // This is generated by the NetabaseModel derive macro
/// impl<'db> RedbModelCrud<'db, MyDef> for User {
///     // Implementation generated by macro
/// }
/// ```
    ...
    Self: 'db
{
    ...
    /// ```rust
    /// // Subscribe to specific topics only
    /// user.create_entry_with_subscriptions(&mut tables, Some(vec![
    ///     SubscriptionKey::Hashtag("rust".into()),
    ///     SubscriptionKey::Hashtag("database".into()),
    /// ]))?;
    /// ```
