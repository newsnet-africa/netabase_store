//! CRUD operations trait for redb-backed models.
//!
//! This module defines the [`RedbModelCrud`] trait, which provides high-level CRUD
//! (Create, Read, Update, Delete) operations for models stored in redb tables.
//!
//! # Overview
//!
//! The `RedbModelCrud` trait handles the complexity of mapping a high-level
//! `NetabaseModel` to the underlying redb tables, including:
//!
//! - **Serialization/Deserialization**: Converting models to/from bytes
//! - **Primary table storage**: Main model data storage
//! - **Secondary index maintenance**: Keeping indexes in sync
//! - **Relational link storage**: Managing foreign key references
//! - **Subscription registration**: Pub/sub topic membership
//! - **Blob chunking**: Automatic chunking of large data fields
//!
//! # Usage
//!
//! This trait is automatically implemented for models that derive `NetabaseModel`.
//! Users interact with it through transaction methods:
//!
//! ```rust
...
//!
//! // Create
//! let user = User { id: UserID("alice".into()), name: "Alice".into(), email: "alice@example.com".into() };
...
//! updated_user.name = "Alice Updated".into();
//! txn.update(&updated_user).unwrap();
//!
//! // Delete
//! txn.delete::<User>(&UserID("alice".into())).unwrap();
//! txn.commit().unwrap();
//! ```
//!
//! # Advanced Patterns
...
//! txn.create_with_subscriptions(&user, Some(vec![SubscriptionKey::UserCreated])).unwrap();
//! txn.commit().unwrap();
//! ```
//!
...
//!
//! let user = User { id: UserID("alice".into()), name: "Alice".into(), email: "alice@example.com".into() };
//! // For models where you want to pre-specify subscriptions
//! let hash = b"custom_hash_value";
//! txn.create_with_subscriptions_and_hash(&user, None, Some(hash)).unwrap();
//! txn.commit().unwrap();
//! ```
...
/// Trait for CRUD operations on redb-backed models.
///
...
///
/// This trait is automatically implemented for types that derive `NetabaseModel`.
/// The macro generates all necessary serialization and table management code.
///
/// # Type Parameters
///
...
/// ```rust
/// use netabase_store::doc_example::*;
/// // This trait is automatically implemented by the NetabaseModel derive macro
...
/// let txn = store.begin_write().unwrap();
/// let user = User { id: UserID("alice".into()), name: "Alice".into(), email: "alice@example.com".into() };
/// txn.create(&user).unwrap();  // Uses the generated RedbModelCrud implementation
/// ```
    ...
    Self: 'db
{
    ...
    /// # Arguments
    ///
    /// * `hash` - Pre-calculated content hash for the model
    /// * `tables` - The opened model tables transaction context
    ///
    /// # Use Case
    ///
    /// Useful for content-addressable storage patterns where models are identified
    /// by their hash and never modified.
    ...
    ///     SubscriptionKey::UserCreated,
    ///     SubscriptionKey::UserUpdated,
    /// ])).unwrap();
    /// txn.commit().unwrap();
    /// ```
    ...
    ///
    /// * `tables` - The opened model tables transaction context
    /// * `subscription_topics` - Optional explicit subscription topics
    /// * `hash` - Optional pre-calculated content hash
    ///
    /// # Note
    ///
    /// This is an internal method. Use `create_entry()`, `create_entry_with_hash()`,
    /// or `create_entry_with_subscriptions()` instead.
    ...
    /// Lists entries using a `ModelKeyRange`, allowing intersecting
    /// constraints across primary and secondary key spaces.
    ///
    /// # Algorithm
    ///
    /// 1. Collect all primary keys within the primary range
    /// 2. For each secondary range, filter the candidate set via intersection
    /// 3. Fetch final models with pagination
    ///
    /// # Type Requirements
    ///
    /// The primary key must implement `Eq + Hash + Clone + Ord` for efficient
    /// set operations and deterministic ordering. Additionally, the primary key's
    /// `SelfType<'a>` must equal the primary key type itself for `.value()` to work.
    fn list_with_key_ranges<'a, 'txn>(
        tables: &'a ModelOpenTables<'txn, 'db, D, Self>,
        ranges: &crate::traits::registry::models::keys::ModelKeyRange<D, Self>,
        config: CrudOptions,
    ) -> NetabaseResult<Vec<AccessGuard<'a, <Self as RedbNetbaseModel<'db, D>>::TableV>>>
    where
        <Self::Keys as NetabaseModelKeys<D, Self>>::Primary: Eq + std::hash::Hash + Clone + Ord,
        <Self::Keys as NetabaseModelKeys<D, Self>>::Secondary: Clone,
        for<'v> <Self::Keys as NetabaseModelKeys<D, Self>>::Primary:
            redb::Value<SelfType<'v> = <Self::Keys as NetabaseModelKeys<D, Self>>::Primary>;
    ...
    /// let store = RedbStore::<ExampleDef>::new_in_memory().unwrap();
    /// let txn = store.begin_write().unwrap();
    /// txn.create(&User { id: UserID("alice".into()), name: "Alice".into(), email: "alice@example.com".into() }).unwrap();
    /// txn.create(&User { id: UserID("bob".into()), name: "Bob".into(), email: "bob@example.com".into() }).unwrap();
    ///
    /// // Iterate over all user entries
    /// let users = txn.list::<User>().unwrap();
    ...
    /// let users = txn.list_range::<User, _>(UserID("a".into())..UserID("c".into())).unwrap();
    /// assert_eq!(users.len(), 2);
    /// ```
    ...
    /// let users = txn.query_secondary::<User, _>(&UserSecondaryKeys::Email("alice@example.com".into())).unwrap();
    ...
    /// let store = RedbStore::<ExampleDef>::new_in_memory().unwrap();
    /// let txn = store.begin_write().unwrap();
    /// // Relational keys are handled automatically by the macro
    /// // This demonstrates that the functionality exists for models that define relations
    /// txn.commit().unwrap();
    /// ```
    ...
    /// let store = RedbStore::<ExampleDef>::new_in_memory().unwrap();
    /// let txn = store.begin_write().unwrap();
    /// // Blob tables are handled automatically by the macro
    /// // Models with blob fields will have this functionality available
    /// txn.commit().unwrap();
    /// ```
    ...
    'db: 'txn
{
    ...
    where
        ...
        for<'v> <Self::Keys as NetabaseModelKeys<D, Self>>::Primary:
            redb::Value<SelfType<'v> = <Self::Keys as NetabaseModelKeys<D, Self>>::Primary>,
    {
        super::range_query::execute_range_query::<D, Self>(tables, ranges, config)
    }
