...
/// if User::has_relational_links(&user) {
///     // Handle links
/// }
/// ```
///
...
where
    ...
    <<<Self as crate::traits::registry::models::model::NetabaseModel<D>>::Keys as crate::traits::registry::models::keys::NetabaseModelKeys<D, Self>>::Libp2p as strum::IntoDiscriminant>::Discriminant: std::fmt::Debug,
{
    type Keys: NetabaseModelKeys<D, Self>;

    const TREE_NAMES: ModelTreeNames<'static, D, Self>
    where
        <<<Self as NetabaseModel<D>>::Keys as NetabaseModelKeys<D, Self>>::Libp2p as strum::IntoDiscriminant>::Discriminant: 'static;
...
// ============================================================================
// Definition Key Conversion
// ============================================================================

/// Trait for models that can convert their primary key to definition-level keys.
///
/// This trait enables type-safe hydration of `RelationalLink`s by providing
/// a way to wrap a model's specific primary key into the definition's key enum,
/// which can then be used with the definition's `dispatch_read` method.
///
/// # How It Works
///
/// 1. Each model has its own primary key type (e.g., `UserID`)
/// 2. Each definition has a `DefKeys` enum containing all model keys
/// 3. This trait provides the conversion: `UserID` → `MyDefKeys::User(UserKeys::Primary(id))`
///
/// # Macro Generation
///
/// This trait is automatically implemented by the `#[netabase_definition]` macro
/// for each model in the definition. You should not implement it manually.
///
/// # Example
///
/// ```ignore
/// // Given a definition with User model:
/// // The macro generates:
/// impl ModelToDefKey<MyDef> for User {
///     fn primary_to_def_key(
///         pk: UserID
///     ) -> MyDefKeys {
///         MyDefKeys::User(UserKeys::Primary(pk))
///     }
///     
///     fn from_def_enum(def: MyDef) -> Option<Self> {
///         match def {
///             MyDef::User(u) => Some(u),
///             _ => None,
///         }
...
///
/// // Usage in hydration:
/// let def_key = User::primary_to_def_key(user_id);
/// let def_enum = MyDef::dispatch_read(&txn, &def_key)?;
/// let user = User::from_def_enum(def_enum)?;
/// ```
pub trait ModelToDefKey<D: NetabaseDefinition>: NetabaseModel<D>
where
    D::Discriminant: 'static + std::fmt::Debug,
    <<Self::Keys as NetabaseModelKeys<D, Self>>::Secondary as IntoDiscriminant>::Discriminant:
        'static + std::fmt::Debug,
    <<Self::Keys as NetabaseModelKeys<D, Self>>::Relational as IntoDiscriminant>::Discriminant:
        'static + std::fmt::Debug,
    <<Self::Keys as NetabaseModelKeys<D, Self>>::Blob as IntoDiscriminant>::Discriminant:
        'static + std::fmt::Debug,
    <<Self::Keys as NetabaseModelKeys<D, Self>>::Subscription as IntoDiscriminant>::Discriminant:
        'static + std::fmt::Debug,
    <<Self::Keys as NetabaseModelKeys<D, Self>>::Libp2p as IntoDiscriminant>::Discriminant:
        'static + std::fmt::Debug,
{
    /// Convert this model's primary key to the definition's key enum.
    ///
    /// This wraps the model-specific primary key into the definition-level
    /// `DefKeys` enum variant, which can then be used with `dispatch_read`.
    fn primary_to_def_key(
        pk: <Self::Keys as NetabaseModelKeys<D, Self>>::Primary,
    ) -> D::DefKeys;

    /// Extract this model from the definition enum.
    ///
    /// Returns `Some(model)` if the definition enum variant matches this model,
    /// `None` otherwise.
    fn from_def_enum(def: D) -> Option<Self>;
}
