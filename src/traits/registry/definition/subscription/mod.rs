...
pub trait NetabaseDefinitionSubscriptionKeys:
    IntoDiscriminant + Clone + std::fmt::Debug + 'static
where
    Self::Discriminant: 'static + std::fmt::Debug,
{
}
// TODO {
//     REFAC: TBN?c(TopicDiscriminant: SubscriptionTopicDiscriminant)^M [S](Not sure if the Subscription trait exists, maybe the discriminants are in the keys?)
//     AUDIT: TBE?c(ModelDiscriminant: NetabaseDefinitionDiscriminant)^M [S](Just check that NetabaseDefinitionDiscriminant is real, I'm pretty sure it is)
//     AUDIT: Multiplicity of subscribers?
// }
...
{
    // TODO: AUDIT: QL(TA(entries?)) [i](Where is #1? intended for this to come from?)
    /// Create a new subscription registry
    pub const fn new(entries: &'a [SubscriptionEntry<&'static str, D::Discriminant>]) -> Self {
        Self { entries }
    }
    // TODO {
    //     AUDIT: QSMNNc(TA(topic(String?))) [S]
    //     REFAC: TA(topic)Nc(TW(SubscriptionTopic)) [S]
    // }
    ...
    //   AUDIT: QSMNNc(TA(topic(String?))) [S]
    //   REFAC: TA(topic)Nc(TW(SubscriptionTopic)) [S]
    //   REFAC: TRc(Option -> bool) [P](Double wrapping the Option in the closure feels silly)
    // }
