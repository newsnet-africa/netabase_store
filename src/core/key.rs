//! Key types for Netabase entry addressing.
//!
//! This module defines the key structures used to uniquely identify and order
//! entries in the Netabase storage system. Keys follow the Netabase Networking
//! Specification for compatibility with peer-to-peer synchronization.
//!
//! # Key Structure
//!
//! A `NetabaseKey` consists of three components:
//! - **Subspace** (32 bytes): Author's public key identifying ownership
//! - **Path** (variable): Semantic path within the namespace
//! - **Rank** (8 bytes): Priority/timestamp for ordering
//!
//! # Path Encoding
//!
//! Paths use tuple encoding for consistent ordering:
//! - Paths can contain hierarchical segments
//! - Ordering is preserved across segments
//! - Supports prefix queries for tree-like data
//!
//! # Example
//!
//! ```rust
...
//!     [0u8; 32],           // Subspace (author pubkey)
//!     b"users/alice".to_vec(), // Path
//!     1234567890,          // Rank (timestamp)
//! );
//!
//! // Keys can be encoded to bytes for storage
//! let bytes = key.to_bytes();
//! let decoded = NetabaseKey::from_bytes(&bytes).unwrap();
