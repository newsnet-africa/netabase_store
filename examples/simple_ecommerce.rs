...
/// Simple E-commerce System Example - Basic Implementation
/// 
/// This example demonstrates basic NetabaseStore usage without complex macros
/// Shows fundamental CRUD operations and data management
...
impl<T> SimpleStore<T> {
    ...
    pub fn insert(&mut self, id: u64, item: T) -> Result<(), String> {
...
pub struct EcommerceApp {
    pub user_store: SimpleStore<User>,
    pub product_store: SimpleStore<Product>,
    pub order_store: SimpleStore<Order>,
    pub review_store: SimpleStore<Review>,
    pub cart_store: SimpleStore<Cart>,
}
...
impl EcommerceApp {
    ...
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        Ok(EcommerceApp {
            user_store: SimpleStore::new(),
            product_store: SimpleStore::new(),
            order_store: SimpleStore::new(),
            review_store: SimpleStore::new(),
            cart_store: SimpleStore::new(),
        })
    }
    ...
    pub fn create_order(&mut self, order: Order) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for item in &order.items {
            if let Some(product) = self.product_store.get(&item.product_id).cloned() {
                let mut updated_product = product;
                updated_product.stock_quantity -= item.quantity;
                updated_product.updated_at = order.created_at;
                self.product_store.insert(updated_product.id, updated_product)?;
            }
        }
        
        self.order_store.insert(order.id, order)?;
        Ok(())
    }
    ...
    pub fn add_to_cart(&mut self, cart_id: u64, product_id: u64, quantity: u32) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Get or create cart
        let mut cart = if let Some(existing_cart) = self.cart_store.get(&cart_id).cloned() {
            existing_cart
        } else {
    ...
    pub fn search_products(
        ...
        if let Some(term) = search_term {
            ...
            products.retain(|p| {
                ...
                p.tags.iter().any(|tag: &String| tag.to_lowercase().contains(&term))
            });
        }
    ...
    pub fn get_sales_analytics(&self) -> Result<SalesAnalytics, Box<dyn std::error::Error>> {
        let orders: Vec<Order> = self.order_store.scan().cloned().collect();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🚀 Simple E-commerce System Demo");
    ...
    println!("\n📈 Store Statistics:");
    println!("Total Users: {}", app.user_store.len());
    println!("Total Products: {}", app.product_store.len());
    println!("Total Orders: {}", app.order_store.len());
    println!("Total Reviews: {}", app.review_store.len());
    println!("Active Carts: {}", app.cart_store.len());
    
    println!("\n✅ Simple e-commerce demo completed successfully!");
    println!("🎉 All basic database operations working perfectly!");
...
mod tests {
    ...
    fn test_user_creation() {
        ...
        assert_eq!(app.user_store.len(), 1);
    }
    ...
    fn test_order_validation() {
        ...
        let updated_product = app.product_store.get(&1).unwrap();
    ...
    fn test_search_functionality() {
        ...
        let laptop = Product {
            id: 1,
            name: "Gaming Laptop".to_string(),
            description: "High-performance laptop for gaming".to_string(),
            price: 150000,
            category: ProductCategory::Electronics,
            stock_quantity: 5,
            seller_id: 1,
            images: vec![],
            tags: vec!["gaming".to_string(), "laptop".to_string()],
            created_at: 1640000000,
            updated_at: 1640000000,
            is_active: true,
        };
        app.add_product(laptop).unwrap();
        ...
        let book = Product {
            ...
            seller_id: 1,
            images: vec![],
            tags: vec!["programming".to_string(), "education".to_string()],
            created_at: 1640000000,
            updated_at: 1640000000,
            is_active: true,
        };
        app.add_product(book).unwrap();
        ...
        assert_eq!(electronics[0].name, "Gaming Laptop");
        
        // Test price range search
        let expensive_items = app.search_products(
            None,
            Some(100000),
            None,
            None,
        ).unwrap();
        assert_eq!(expensive_items.len(), 1);
        
        // Test text search
        let programming_items = app.search_products(
            None,
            None,
            None,
            Some("programming"),
        ).unwrap();
        assert_eq!(programming_items.len(), 1);
        assert_eq!(programming_items[0].name, "Programming Book");
    }
}