...
pub fn generate_repository_cli(repo_name: &Ident, definitions: &[(Ident, Vec<Ident>)]) -> TokenStream {
    ...
    let definition_cli_gens = definitions.iter().map(|(def_name, models)| {
        generate_definition_cli(def_name, models)
    });
...
pub fn generate_store_cli(store_name: &Ident, definitions: &[(Ident, Vec<Ident>)]) -> TokenStream {
    let repo_cli = generate_repository_cli(
        &quote::format_ident!("{}Commands", store_name),
        definitions
    );
