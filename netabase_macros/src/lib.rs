...
// Note: We use the full DefinitionSchema from utils::schema
// This ensures compatibility with schema.toml files generated by export_toml()
...
pub fn infer_netabase_definition(input: TokenStream) -> TokenStream {
    ...
    let schema: crate::utils::schema::DefinitionSchema = match toml::from_str(&content) {
        Ok(s) => s,
        Err(e) => {
